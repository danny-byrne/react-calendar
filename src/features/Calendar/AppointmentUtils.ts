...
export const getTimeIndicies = (startTimeNumber: string, endTimeNumber: string) => {
    const startTime = startTimeNumber;
    const endTime = endTimeNumber;
    const startIndex = timeNumbersDropDownOptions.findIndex((val) => val.key === startTime);
    const endIndex = timeNumbersDropDownOptions.findIndex((val) => val.key === endTime);

    return [startIndex, endIndex];
};

export const getAddAppointmentMutationValues = (values: AppointmentFormikValues) => {
    const appointmentValues = getBaseAppointmentMutationValues(values);
    return { variables: { input: appointmentValues } };
};

export const getUpdateAppointmentMutationValues = (values: AppointmentFormikValues, id: string) => {
...
const getBaseAppointmentMutationValues = (values: AppointmentFormikValues) => {
    const { name, startDate, startTimeAmPm, startTimeNumber, endDate, endTimeAmPm, endTimeNumber, address } =
        values.appointment;

    const startDateTime = convertValuesToDateTimeString(startDate, startTimeNumber, startTimeAmPm);
    const endDateTime = convertValuesToDateTimeString(endDate, endTimeNumber, endTimeAmPm);
...
export const validateAppointmentValues = (values: AppointmentFormikValues) => {
    const errors: any = {};
    const { name, startDate, startTimeAmPm, startTimeNumber, endDate, endTimeAmPm, endTimeNumber } = values.appointment;
    ...
    if (startDate.getDate() === endDate.getDate()) {
        if (startTimeAmPm === endTimeAmPm) {
            const [startIndex, endIndex] = getTimeIndicies(startTimeNumber, endTimeNumber);
...
export const convertValuesToDateTimeString = (date: Date, timeNumber: string, amPm: string) => {
...
export const getTimeText = ({ startDateTime, endDateTime }: IGetTimeTextInput) => {
    let timeText = '';
    if (startDateTime && endDateTime) {
        const [startDateTimeHours, startDateTimeAmPm, startDateTimeMinutesString] = getTimeStrings(startDateTime);
        const [endDateTimeHours, endDateTimeAmPm, endDateTimeMinutesString] = getTimeStrings(endDateTime);
...
const getTimeStrings = (dateString: string): [number, string, string] => {
    ...
    let [dateTimeHours, dateTimeMinutes] = [dateTime.getHours(), dateTime.getMinutes()];
    let dateTimeAmPm = 'AM';
    ...
    const dateTimeMinutesString = dateTimeMinutes < 10 ? `0${dateTimeMinutes.toString()}` : dateTimeMinutes.toString();

    return [dateTimeHours, dateTimeAmPm, dateTimeMinutesString];
};

export const getEndTimeValues = (item: IComboBoxOption, startTimeAmPm: string): [IComboBoxOption, string] => {
    const itemIndex = timeNumbersDropDownOptions.findIndex((val) => val.key === item.key);
    let endTimeIndex;
...
export const getAppointmentEditInitialValues = (appointment: Appointment) => {
    ...
    const startTimeNumber = `${startHours}:${startMinutes === '0' ? '00' : startMinutes}`;
...
export const isOccuringOn = (appointment: Appointment, selectedDate: Date) => {
    ...
    switch (freq) {
        case 'DAILY': {
            const daysDifference = Math.floor((selectedDateTime - appointmentStartTime) / oneDay);
            return daysDifference % interval === 0;
        }

        case 'WEEKLY': {
            const weeksDifference = Math.floor((selectedDateTime - appointmentStartTime) / oneWeek);
            return weeksDifference % interval === 0 && selectedDate.getDay() === appointmentStartDate.getDay();
        }
        ...
        case 'MONTHLY': {
            ...
            return monthsDifference % interval === 0 && selectedDate.getDate() === appointmentStartDate.getDate();
        }

        case 'YEARLY': {
            const yearsDifference = selectedDate.getFullYear() - appointmentStartDate.getFullYear();
            return (
