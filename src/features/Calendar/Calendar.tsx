...
import { months, monthsFullNames, weekdays, weekdaysFull } from 'src/utils/utils';
...
import { useGetTimezoneInfo, getAdjustedTime } from 'src/common/hooks/useGetTimezoneInfo';
...
const Calendar: React.FC = () => {
    ...
    useEffect(() => {
        if (data && selectedDate) {
            const filteredAppointments = data?.careRecipientAppointments?.appointments.filter((appointment) => {
    ...
    const AppointmentCard: React.FC<IExampleAppointmentProps> = ({ titleText, timetext, locationName }) => {
        return (
            <Stack>
                <Stack horizontal tokens={{ childrenGap: 12 }}>
                    <div className={classNames['wc-Calendar--appointmentBar']} />
                    <Stack>
                        <Text className={classNames['wc-Calendar--appointmentText']}>{titleText}</Text>
                        <Text className={classNames['wc-Calendar--appointmentTime']}>{timetext}</Text>
                    </Stack>
                </Stack>
                {locationName && (
                    <Text className={classNames['wc-Calendar--appointmentLocationText']}>{locationName}</Text>
                )}
            </Stack>
        );
    };
    ...
    const buttonPropsList = appointments.map((appointment) => {
        ...
        return {
            ...
            content: <AppointmentCard titleText={appointment.description} timetext={timeText} />,
        };
    });
