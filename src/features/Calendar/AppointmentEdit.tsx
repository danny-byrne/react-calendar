...
const AppointmentEdit: React.FC<IAppointmentEditProps> = ({ onDismiss, appointment }) => {
    ...
    const [updateAppointment, { loading }] = useAppointmentUpdateMutation({
        ...
        onCompleted: () => {
            navigate(RouterConfig.Appointment(appointment.id) + '?status=edited', { replace: true });
        },
        onError: () => {
        ...
        <>
            <Formik
                ...
                onSubmit={async (values) => {
                    ...
                    const updateAppointmentValues = await getUpdateAppointmentMutationValues(values, appointment.id);
