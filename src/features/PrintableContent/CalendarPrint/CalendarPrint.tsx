...
import { getClassNames, getScheduleGridClassNames } from '../PrintableContentContainer.classNames';
...
import { createHourLabels, renderEvents, generateGridLines } from './CalendarPrintElements';

import { formatDateRange, determineEventPositioning, getTimeRangeForWeek, getGridDimensions } from './helpers';
...
const CalendarPage: React.FC<ICalendarPagePrintProps> = ({ pageNumber, includePageBreak, week, recipientData }) => {
    const dateRangeLabel = formatDateRange(week.startDay, recipientData.timeZone);

    return (
        <>
            {pageNumber > 1 && <div className={classNames['wc-PrintableContent--headerBuffer']} />}

            <Stack className={classNames['wc-PrintableContent--PageContainer']} verticalAlign="space-between">
                <div>
                    <PrintablePageHeader recipientData={recipientData} />
                    <div className={classNames['wc-PrintableContent--timeframeText']}>{dateRangeLabel}</div>
...
const ScheduleSection: React.FC<IScheduleSectionProps> = ({ week }) => {
    ...
    const { gridTemplateColumns, gridTemplateRows, numRows, numColumns } = getGridDimensions(range, daysWithEvents);
    ...
    return (
        <div className={scheduleGridClassNames['wc-PrintableContent--scheduleContainer']}>
            {hoursDisplay}
            {events}
            {gridLines}
        </div>
