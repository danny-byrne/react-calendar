...
export const getStartWeekBasedOnIncomingEventDates = (
  ...
  const dateOfMondayOfFirstEventWeek =
    new Date(firstEventDate).getDate() - (firstEventDayOfWeek - 1);
  const startDay = new Date(firstEventDate).setDate(
    dateOfMondayOfFirstEventWeek
  );
...
export const sortEventData = (
  events: ICalendarEvents
): ISortEventDataReturnValue => {
...
const findStartDateAndWeeksRange = (
  events: ICalendarEvents
): IFindStartDateAndWeeksRangeReturnValue => {
  const lastEventDate = events[events.length - 1]?.startDateTime;
  const lastEventDayOfWeek = new Date(lastEventDate).getDay();
  const dateOfSundayOfLastEventWeek =
    new Date(lastEventDate).getDate() + (7 - lastEventDayOfWeek);
  const endDateTime = new Date(lastEventDate).setDate(
    dateOfSundayOfLastEventWeek
  );
...
const setKeyedEventsByDate = (
  events: ICalendarEvents
): IEventsKEyedByDatesReturnValue => {
...
const setWeeklyEvents = ({
  eventsKeyedByDates,
  startDate,
  numWeeks,
}: ISetWeeklyEventsInputValue): IWeek[] => {
  let weeks = [];
...
export interface IFilterValues
  extends IMedicationsPrintFilterValues,
...
export const createMedicationCalendarEventsBasedOnMedicationsData = (
  ...
  prescriptions.forEach((prescription) => {
    const eventBlocks: ICalendarEvents = createEventsForMedSchedules(
      prescription,
      filterValues
    );
...
const createEventsForMedSchedules = (
  ...
  const dateRange: Array<Date> = createDateRangeForMedicationSchedule({
    prescription,
    filterValues,
  });

  const medScheduleEvents: ICalendarEvents = createEventsBasedOfScheduleBlocks({
    dateRange,
    prescription,
  });
...
const createDateRangeForMedicationSchedule = ({
  ...
  const { startDateForComparison, endDateForComparison } =
    getStartEndDateForMedication({
      prescription,
      filterValues,
    });

  // eslint-disable-next-line no-unsafe-optional-chaining
  const { daysOfWeekParsed, recurrenceFrequencyCount, frequency } =
    prescription?.rRuleParams;
...
const getDateRange = ({
  ...
  while (dateForComparison <= endDateForComparison) {
    ...
    if (frequency === Frequency.MONTHLY) {
      dateForComparison.setMonth(
        dateForComparison.getMonth() + recurrenceFrequencyCount
      );
    } else if (frequency === Frequency.YEARLY) {
      dateForComparison.setFullYear(
        dateForComparison.getFullYear() + recurrenceFrequencyCount
      );
    } else {
      let dayIncrementValue = getScheduleIncrementValue(
        frequency,
        recurrenceFrequencyCount
      );

      dateForComparison.setDate(
        dateForComparison.getDate() + dayIncrementValue
      );
    }
  }
...
const createEventsBasedOfScheduleBlocks = ({
  ...
  dateRange.forEach((date) => {
    const {
      medicationBlockStartDateTime,
      medicationBlockEndDateTime,
...
const getMedStartEndDateTimesAndDescription = ({
  ...
  const medicationBlockStartDateTime = createDateTimeForMedicationBlockEvent(
    date,
    prescription
  );
  ...
  medicationBlockEndDateTime.setMinutes(
    medicationBlockEndDateTime.getMinutes() + MEDICATION_BLOCK_MINUTES
  );
  const medicationDescription = `${prescription?.medication?.name} ${
...
const getStartEndDateForMedication = ({
  ...
  const startDateForComparison =
    scheduleStartDate > printRangeStart ? scheduleStartDate : printRangeStart;

  let endDateForComparison;
  if (scheduleEndDate) {
    endDateForComparison =
      scheduleEndDate < printRangeEnd ? scheduleEndDate : printRangeEnd;
  } else {
...
const createDateTimeForMedicationBlockEvent = (
...
export const determineEventPositioning = (
  day: DayBlock,
  range: string[]
): DayBlock => {
  ...
  for (let i = 0; i < positionedEvents.length; i++) {
    ...
    if (i < positionedEvents.length - 1) {
      for (let j = i + 1; j < positionedEvents.length; j++) {
        ...
        const currentEventHasSameStartTimeAsNext =
          currentEventStartTimeIndex === nextEventStartTimeIndex;

        const currentEventEndsBeforeNextEvent =
          currentEventEndTimeIndex <= nextEventStartTimeIndex;

        const currentEventOverlapsNext =
          !currentEventHasSameStartTimeAsNext &&
          currentEventEndTimeIndex > nextEventStartTimeIndex;

        if (
          currentEventHasSameStartTimeAsNext ||
          currentEventEndsBeforeNextEvent
        ) {
...
const getEarliestAndLatestTimesAndIndexes = (
  days: DayBlock[]
): IGetEarliestAndLatestTimesAndIndexesReturnValue => {
...
export const getTimeRangeForWeek = (
  days: DayBlock[]
): IGetTimeRangeForWeekReturnValue => {
  let { earliestStartTimeIndex, latestEndTimeIndex } =
    getEarliestAndLatestTimesAndIndexes(days);
  ...
  const range = DAY_LENGTH_FIFTEEN_MINUTE_INCREMENTS.slice(
    earliestStartTimeIndex,
    latestEndTimeIndex + 1
  );
