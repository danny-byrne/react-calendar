...
const CalendarPrintPanel: React.FC<ICalendarPrintPanelProps> = ({
  onDismiss,
  appointments,
}) => {
  ...
  defaultEndDate.setDate(
    defaultEndDate.getDate() + DEFAULT_FILTER_DATE_RANGE_NUM_DAYS
  );
  ...
  useEffect(() => {
    ...
    if (
      medicationsData?.careRecipientMedicationPrescriptions?.prescriptions
        .length &&
      filterValues.showMedSchedule
    ) {
      const { prescriptions } =
        medicationsData.careRecipientMedicationPrescriptions;
      ...
      const medicationsWithParsedRRuleData =
        medicationsFilteredByActiveWithDosageInfo.map((prescription) => {
