...
const PrintablePageHeader: React.FC<IPrintablePageHeaderProps> = ({ recipientData }) => {
    return (
        <>
            <Stack className={classNames['wc-PrintableContent--headerContent']} verticalAlign="space-between">
...
const PersonalDetailsHeaderSection: React.FC<IPrintablePageHeaderProps> = ({ recipientData }) => {
    return (
        <Stack horizontal verticalAlign="center" tokens={{ childrenGap: 10 }}>
            ...
            />
            <Stack tokens={stackTokens} className={classNames['wc-PrintableContent--headerContent--details']}>
                <div className={classNames['wc-PrintableContent--semiBoldText']}>{recipientData.name}</div>
...
const EmergencyContactHeaderSection: React.FC<IPrintablePageHeaderProps> = ({ recipientData }) => {
    ...
    return (
        <Stack tokens={stackTokens}>
            {emContactName && (
                <>
                    <div className={classNames['wc-PrintableContent--semiBoldText']}>Emergency Contact:</div>
                    <div className={classNames['wc-PrintableContent--EmContactName']}>{emContactName}</div>
                    <div>{formatPhoneNumber(recipientData.emergencyContact.phoneNumber)}</div>
                </>
            )}
        </Stack>
    );
