...
const createTitleColumns = (titles) => {
  return (
    <Stack
      horizontal
      className={classNames["wc-CareRecipientPrint--columnNamesContainer"]}
    >
      {titles.map((title) => {
        return (
          ...
          <div
            key={title}
            className={
              classNames["wc-CareRecipientPrint--MedicationsGridLabel"]
            }
          >
            {title}
          </div>
...
const createBlock = ({ rows, label, titleColumns, rowHeight }) => {
  ...
  return (
    <>
      <div className={classNames["wc-CareRecipientPrint--LabelContainer"]}>
        {label}
      </div>
                ...
                {values.map((content) => {
                  return (
                    <Stack
                      key={Math.random()}
                      className={
                        classNames["wc-CareRecipientPrint--GridContent"]
                      }
...
const CareRecipientPrint = (props: IMedicationsPrintProps) => {
  ...
  const CareRecipientPage: React.FC<IMedicationsPageProps> = ({
    pageNumber,
    blocks,
  }) => {
    ...
    return (
      <>
        {pageNumber > 1 && (
          <div className={classNames["wc-CareRecipientPrint--headerBuffer"]} />
        )}

        <Stack
          className={classNames["wc-CareRecipientPrint--PageContainer"]}
          verticalAlign="space-between"
        >
          <div>
            <PrintablePageHeader recipientData={recipientData} />
            <Stack tokens={{ childrenGap: PRINT_ELEMENT_GAP }}>
              {pageBlocks}
            </Stack>
          </div>
          <PrintablePageFooter
