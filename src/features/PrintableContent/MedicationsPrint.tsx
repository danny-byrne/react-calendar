...
import {
  getClassNames,
  ACTIVITY_TOKEN_GAP,
} from "./MedicationsPrint.classNames";
...
const MedicationsPrint = ({
  recipientData,
  medications,
  titleColumns,
  pageLabel,
}: IMedicationsPrintProps) => {
  ...
  const medicationTitleColumns = (
    <Stack
      horizontal
      className={classNames["wc-MedicationsPrint--columnNamesContainer"]}
    >
      {titleColumns.map((title, i) => {
        return (
          ...
          <div
            key={`title-${i}`}
            className={classNames["wc-MedicationsPrint--MedicationsGridLabel"]}
          >
            {title}
          </div>
        );
  ...
  const HeaderContent = () => (
    <>
      ...
      <div
        className={classNames["wc-MedicationsPrint--MedicationsLabelContainer"]}
      >
  ...
  const MedicationsBlock = ({ medications }) => {
    ...
    return (
      <Stack
        className={classNames["wc-MedicationsPrint--MedicationsGrid"]}
        tokens={stackTokens}
      >
        <Stack tokens={{ childrenGap: ACTIVITY_TOKEN_GAP }}>
          {pageLabel === MEDICATION_PRINT_PAGE_LABELS.All && (
            <div
              className={
                classNames["wc-MedicationsPrint--allMedicationsStatusLabel"]
              }
            >
              Current
            </div>
          ...
          {activeMeds.map((medication, i) => {
            ...
            return (
              <>
                <Stack
                  className={
                    classNames["wc-MedicationsPrint--MedicationContainer"]
                  }
                  ...
                  <Stack horizontal>
                    {columns.map((content, i) => {
                      return (
                        <Stack
                          className={
                            classNames["wc-MedicationsPrint--MedicationContent"]
                          }
  ...
  const MedicationsPage: React.FC<IMedicationsPageProps> = ({
    medications,
    pageNumber,
  }) => {
    return (
      <>
        {pageNumber > 1 && (
          <div className={classNames["wc-MedicationsPrint--headerBuffer"]} />
        )}

        <Stack
          className={
            classNames["wc-MedicationsPrint--MedicationsPageContainer"]
          }
