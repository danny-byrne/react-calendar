...
import { getClassNames, ACTIVITY_TOKEN_GAP } from './ActivitiesPrint.classNames';
...
const titleColumns = (
    <Stack horizontal className={classNames['wc-ActivitiesPrint--columnNamesContainer']}>
        {COLUMN_TITLES.map((title) => {
            return (
                <div key={title} className={classNames['wc-ActivitiesPrint--ActivitiesGridLabel']}>
                    {title}
                </div>
            );
        })}
...
const ActivitiesPrint = (props: IActivitiesPrintProps) => {
    ...
    const HeaderContent: React.FC<IHeaderProps> = ({ showActivitiesLabel }) => (
        <>
            ...
            <div className={classNames['wc-ActivitiesPrint--ActivitiesLabelContainer']}>
                {showActivitiesLabel && (
                    <Stack horizontal verticalAlign="center" tokens={stackTokens}>
                        <Icon iconName="Family" className={classNames['wc-ActivitiesPrint--familyIcon']} />
    ...
    const ExperiencesBlock = ({ experiences }) => {
        return (
            <Stack className={classNames['wc-ActivitiesPrint--ActivitiesGrid']} tokens={stackTokens}>
                <Stack tokens={{ childrenGap: ACTIVITY_TOKEN_GAP }}>
                    {experiences.map((experience) => {
                        const { details, availability, phoneNumber, title, id: experienceId } = experience;

                        const careCircleMembers = experience?.careCircle?.careCircleMembers || [];

                        const participatingCareCircleMembers = careCircleMembers.filter((member) => {
                            const memberExperiencesIds = member.experienceOccurrences.map((experience) => {
                                return experience.experience.id;
                            });
                            return memberExperiencesIds.includes(experienceId);
                        });

                        const activityCareCircleMembers = participatingCareCircleMembers.map((member) => {
                            return { image: member.careGiver.imageBase64, name: member.careGiver.displayName };
                        });

                        const formattedPhoneNumber = formatPhoneNumber(phoneNumber);

                        const experienceColumns = [title, formattedPhoneNumber, availability];
                        return (
                            <>
                                <Stack
                                    className={classNames['wc-ActivitiesPrint--ActivityContainer']}
                                ...
                                >
                                    <Stack horizontal>
                                        {experienceColumns.map((content) => {
                                            return (
                                                <Stack
                                                    className={classNames['wc-ActivitiesPrint--ActivityContent']}
                                        ...
                                        <Stack
                                            className={classNames['wc-ActivitiesPrint--ActivityContent-Details']}
                                            verticalAlign="center"
                                        >
                                            {details}
                                        </Stack>
                                    ...
                                    <Stack
                                        horizontal
                                        className={classNames['wc-ActivitiesPrint--ActivityMembersContent']}
                                        verticalAlign="center"
                                        tokens={stackTokens}
                                    >
    ...
    const ActivityPage: React.FC<IActivityPageProps> = ({ experiences, pageNumber }) => {
        return (
            <>
                {pageNumber > 1 && <div className={classNames['wc-ActivitiesPrint--headerBuffer']} />}
    ...
    return (
        <div className={classNames['wc-ActivitiesPrint--innerContent']}>
            {chunkedExperiences.map((chunk, i) => {
                ...
                return <ActivityPage experiences={chunk} pageNumber={i + 1} key={`key-${i}`} />;
            })}
        </div>
    );
};
