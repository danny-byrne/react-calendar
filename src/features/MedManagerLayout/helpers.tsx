...
import { MedicationWithRRule, getFrequencyHeaderAndContent } from '../Medications/helpers';
...
const DEFAULT_MEDS_COLUMN_TITLES = ['Name & Strength', 'Instructions', 'Notes', 'Provider', 'Pharmacy'];
const REFILL_MEDS_COLUMN_TITLES = ['Name & Strength', 'Next refill', 'Pharmacy', 'Provider'];

export const todaysDate = new Date();

export const todaysDateAtMiddayUTC = getDateAtMidday(todaysDate).toUTCString();
export const todaysDateAtMidnightMilliseconds = new Date(todaysDate).setUTCHours(0, 0, 0, 0);
...
export const sortMedsToDisplayBasedOnFilters = (medData, filter) => {
    ...
    const medsPrintDisplay = getMedicationPrintDisplayValues(filteredMeds, selectedFilter);

    const { pageLabel, titleColumns } = getPageLabelAndTitleColumns(selectedFilter);

    return { medsPrintDisplay, titleColumns, pageLabel };
};

const filterMedsBasedOnIncomingFilters = ({ meds, selectedFilter, filterValues }) => {
    const filteredMeds = meds.filter((med) => {
        ...
        const medIsOutOfDateRange = startDateIsAfterFilterDateEnd || endDateBeforeFilterDateStart;
        const medHasAStartDateAndIsWithinFilterDatesRange = startDate && !medIsOutOfDateRange;
        ...
        const filterOptionIsRefillsAndMedHasRefills = selectedFilter === selectOptions.Refills && medHasRefills;
...
const getMedicationPrintDisplayValues = (filteredMedications, selectedFilter) => {
    const medPrintDisplayValues = filteredMedications.map((medication) => {
        ...
        const pharmacyDetails = [pharmacyName, pharmacyLocation, pharmacyPhoneNumber].filter(
        ...
        const allMedsColumns = [medName, instructions, notes, providerBlock, pharmacyBlock];
        const refillMedsColumns = [medName, nextRefillDate, pharmacyBlock, providerBlock];
        const medicationColumns = selectedFilter === selectOptions.Refills ? refillMedsColumns : allMedsColumns;
...
const createMedInstructions = (med) => {
    const { frequencyText, dosageInformation } = getFrequencyHeaderAndContent(med);
...
const extractScheduleInformation = ({ medication, filterValues }: IExtractMedicationInformationInput) => {
    ...
    const values = {
        ...
        medIsDailySchedule: rRuleParams?.frequency === Frequency.DAILY && !medIsRepeatDays,
    ...
    values.medHasEnded = new Date(todaysAbsoluteDateInMilliseconds) > values.endDateToCompare;

    return values;
};

const extractMedicationInformation = (medication) => {
    const medName = `${medication.medication.name}  ${medication?.strengthValue ?? ''}`;
    const nextRefillDate = medication?.refills[0]?.refillDate;
    const pharmacyName = medication?.refills[0]?.pharmacy?.name;
    const pharmacyLocation = medication?.refills[0]?.pharmacy?.location?.singleLineAddress;
    const pharmacyPhoneNumber = formatPhoneNumber(medication?.refills[0]?.pharmacy?.phoneNumber);
    const providerName = getProviderName(medication?.prescribingProvider);
    const instructions = createMedInstructions(medication);
    const conditionName = medication?.takenFor?.condition?.name ? medication?.takenFor?.condition?.name : '';
    const directions = medication?.directions ?? '';
    const notes = conditionName + ' ' + directions;
    const providerAddress = medication?.prescribingProvider?.singleLineAddress ?? null;
