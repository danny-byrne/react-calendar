...
const getHealthHistoryItemDate = ({
  dateDay,
  dateMonth,
  dateYear,
  dateRelativePeriodStart,
}) => {
...
const determineIfConditionHasBeenEnteredAsUnsureOfDates = (
  condition
): boolean => {
  return (
...
const sortCareRecipientConditions = ({
  conditionsData,
  startDate,
  endDate,
}) => {
  ...
  const sortedConditions = conditions
    .filter((condition) => {
      ...
      const conditionHasBeenEnteredAsUnsureOfDates =
        determineIfConditionHasBeenEnteredAsUnsureOfDates(condition);
      const conditionHasRelativeTimePeriod =
        condition?.conditionRelativeTimePeriod;
      const conditionIsWithinDateFilterRanges =
        conditionDate > startDate && conditionDate < endDate;
      return (
    ...
    .map((condition) => {
      ...
      if (conditionHasBeenEnteredAsUnsureOfDates) {
        ...
        return {
          ...
          date: `Relative time period : ${snakeCaseToCapitalized(
            conditionHasRelativeTimePeriod
          )}`,
        };
      } else {
        const { absoluteDatesAndRelativeTimeframes } =
          getTimeFrameText(condition);
        ...
        const { labelText, yearsText } =
          absoluteDatesAndRelativeTimeframes[
            absoluteDatesAndRelativeTimeframes.length - 1
          ];
        //todo: implement allergy notes when that feature exists
        return {
          name: condition.condition.name,
          date: `${labelText + yearsText}`,
        };
      }
    });
...
const sortCareRecipientImmunizations = ({
  immunizationsData,
  startDate,
  endDate,
}) => {
  const immunizations =
    immunizationsData?.careRecipientImmunizations?.immunizations ?? [];
  ...
  const sortedImmunizations = immunizations
    .filter((immunization) => {
      const immunizationDate = getHealthHistoryItemDate({
        ...
        dateRelativePeriodStart:
          immunization.immunizationDateRelativePeriodStart,
      });
      const immunizationIsWithinDateRangeFilters =
        immunizationDate > startDate && immunizationDate < endDate;
      return (
        immunization.recordStatus == RecordStatus.Active &&
        immunizationIsWithinDateRangeFilters
      );
    })
    .map((immunization) => {
      return {
        name: immunization.vaccineProductAdministered.name,
        date: immunization.immunizationDateYear,
      };
    });
  ...
  const sortedAllergies = allergies
    ...
    .map((allergy) => {
      ...
      return {
        ...
        severity: capitalizeOnlyFirstLetter(allergy.severity),
        notes: "",
      };
    });
...
export const useGetDataForPrintPages = (): IRecipientDataType => {
  ...
  const photoURL =
    photoData?.careRecipientPhoto?.careRecipientImageURL ?? defaultProfile;

  const birthday = dataProfile?.dOB
    ? getDateAtMidday(new Date(dataProfile?.dOB)).toLocaleDateString(
        locale,
        dateOptions
      )
    : "";
  ...
  const emergencyContact =
    careTeamData?.usersCareCircle?.careCircleMembers?.filter(
...
export const useGetHealthHistoryDataForPrintPages = (
  startDateFilter,
  endDateFilter
): IHealthHistoryDataType => {
  const { data: careRecipientProfileData } = useGetCareRecipientProfileQuery();
