...
export const getDocumentCardDisplayDate = (input) => {
    ...
    if (dateInput < lastWeekDate) {
        ...
        displayDate = `${DAYS[dateInput.getDay()]} at ${getDocumentTimeDisplay(dateInput)}`;
    }
    return displayDate;
};
...
export const getDocumentDetailDisplayDate = (input) => {
    ...
    let displayDate = `${months[dateInput.getMonth()]} ${dateInput.getDate()}, ${dateInput.getFullYear()}`;
...
export const timeZonesToDropdownOptions = (timezoneData: GetAvailableUserTimeZonesQuery): IDropdownOption[] => {
    const timezoneDropdownOptions = timezoneData.availableUserTimeZones.reduce((allTimezones, currentTimezones) => {
