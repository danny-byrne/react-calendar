import { RelationshipsToLovedOne, Month } from 'src/graphQL/serverMocks/graphQLGeneratedCode';
...
const getRelationshipText = (relationshipToLovedOne: RelationshipsToLovedOne, blankIfNotSet: boolean = false) => {
    const defaultText = blankIfNotSet ? '' : 'Edit to set a relationship';
    const relationship = familyRelationshipDropdownOptions.filter((obj) => obj.key === relationshipToLovedOne);
...
const getTimeFrameDropdownOptions = ({ timeframe, relativeStart, relativeEnd }) => {
    const options = [
        { key: RelativeDate.InTheLast5Years, text: `Since ${fiveYearsAgo} (within previous 5 years)` },
        { key: RelativeDate.InTheLast5to10Years, text: `${tenYearsAgo} - ${fiveYearsAgo} (5 to 10 years ago)` },
        { key: RelativeDate.MoreThan10YearsAgo, text: `Before ${tenYearsAgo} (more than 10 years ago)` },
    ];
    if (timeframe === RelativeDate.Custom) {
        options.unshift({ key: RelativeDate.Custom, text: `Between ${relativeStart} and ${relativeEnd}` });
    }
    return options;
};
...
const getTimeZoneText = (careGiver: CareGiver, canEditTimeZone: boolean): string => {
    let defaultText = canEditTimeZone ? 'Edit to set a time zone' : 'No timezone set';
...
const generateUniqueId = () => Math.floor(Math.random() * Math.floor(Math.random() * Date.now()));
