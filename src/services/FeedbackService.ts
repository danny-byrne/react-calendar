...
export const useFeedbackService = (): IFeedbackService => {
  ...
  return {
    ...
    hasToast:
      state.hasErrorToast || state.hasSuccessToast || state.hasInfoToast,
    ...
    setErrorFullscreen: (message: string) =>
      dispatch({
        type: DISPATCH_MESSAGES.setErrorFullscreen,
        payload: { message: message },
      }),
    clearErrorFullscreen: () =>
      dispatch({ type: DISPATCH_MESSAGES.clearErrorFullscreen }),
    setErrorToast: (
      message: string,
      title?: string,
      actionText?: string,
