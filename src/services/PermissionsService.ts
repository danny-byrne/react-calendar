    ...
    return {
        ...
        setAgreements: (
            ...
            if (agreesToTermsAndPrivacy) {
                localStorage.setItem(LOCAL_STORAGE_KEYS.AGREE_TO_TOS, agreesToTermsAndPrivacy);
            }

            if (understandsIntendedAppUse) {
                localStorage.setItem(LOCAL_STORAGE_KEYS.AGREE_UNDERSTANDS_USAGE, understandsIntendedAppUse);
            }

            if (understandsMicrosoftUseOfTheirData) {
                localStorage.setItem(LOCAL_STORAGE_KEYS.AGREE_UNDERSTANDS_DATA_USE, understandsMicrosoftUseOfTheirData);
            }
            ...
            if (agreesToOpenAiUse) {
                localStorage.setItem(LOCAL_STORAGE_KEYS.AGREE_TO_OPENAI, agreesToOpenAiUse);
            }
        },
        getAgreements: () => {
            ...
            const agreesToTermsAndPrivacy = localStorage.getItem(LOCAL_STORAGE_KEYS.AGREE_TO_TOS);
            const understandsIntendedAppUse = localStorage.getItem(LOCAL_STORAGE_KEYS.AGREE_UNDERSTANDS_USAGE);
            const understandsMicrosoftUseOfTheirData = localStorage.getItem(
                LOCAL_STORAGE_KEYS.AGREE_UNDERSTANDS_DATA_USE,
            );
            const understandsNotPermittedToUsePlatformForMinors = localStorage.getItem(
                LOCAL_STORAGE_KEYS.UNDERSTANDS_MINORS_NOT_PERMITTED,
            );
            const agreesToOpenAiUse = localStorage.getItem(LOCAL_STORAGE_KEYS.AGREE_TO_OPENAI);
            return {
                agreesHasConsentToManageLoveOnesHealth,
